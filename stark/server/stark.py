...
class ModelList:
    def __init__(self, model_stark_obj, queryset):
        ...
        self.group_tag_show = self.model_stark_obj.group_tag_show
    ...
    def get_head(self):
        ...
        if not self.model_stark_obj.list_display:
            head_list = [mark_safe("<input class='total-check' type='checkbox'>"),
                         self.model_stark_obj.model._meta.model_name, "操作", "操作"]
        else:
        ...
        if self.group_filter:
            for filter_option_obj in self.group_filter:
                ...
                if isinstance(field_obj, ForeignKey) or isinstance(field_obj, ManyToManyField):
                    filter_row_obj = FilterRow(filter_option_obj.get_queryset(field_obj), filter_option_obj,
                                               self.request)
                else:
                    filter_row_obj = FilterRow(filter_option_obj.get_choices(field_obj), filter_option_obj,
                                               self.request)
...
class ModelStark:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if pop_up_id:
                    ...
                    ret = {"status":False,"pop_up_id": pop_up_id, "pk": None, "model_name": None}
                    model_name = request.GET.get('mn')  # customer
                    related_name = request.GET.get('rn')  # consultant, "None"

                    for related_object in model_obj._meta.related_objects:
                        ...
                        if model_name == _model_name and related_name == str(_related_name):
                            # print("999999999999")
                            result = self.model.objects.filter(**_limit_choices_to, pk=model_obj.pk)
                            if result:  # 通过校验
                                ret['status'] = True
                                ret['model_name'] = str(model_obj)
                                ret['pk'] = getattr(model_obj, _field_name)
                                return render(request, "pop_post.html",
                                              {"ret": json.dumps(ret, ensure_ascii=False)})
                    return render(request, "pop_post.html", {"ret": json.dumps(ret, ensure_ascii=False)})
                else:
                    list_url = self.get_list_url()
    ...
    # def cancel_course(self, request, cust_id):
    #
    #     return HttpResponse("10101011")
    ...
    def get_urls(self):
        ...
        if hasattr(self, "get_extra_url"):
            # print("++++++++++++++++++++++", self.extra_url)
            url_temp.extend(self.get_extra_url())
            # url_temp.extend([url(r"^cancel_course/(\d+)/(\d+)/", self.wrapper(self.cancel_course))])
            # print(url_temp)
        return url_temp
